...
{      
	...
	while (run_state)
	{
        ...
        if(is_edit_open) show_dialog(&edit_window);
        ...
        switch (dialog_state)//处理对话框事件
        {
            ...
            case DIALOG_EXIT://退出对话框
                dialog_return = show_dialog(&exit_dialog);
                ...
                break;
            case DIALOG_OPEN:
                dialog_return = show_dialog(&open_dialog);
                if (dialog_return)
                {
                    dialog_state = DIALOG_EMPTY;//设为无对话框状态
                    show_menu(BLUE);
                ...
                break;
            case DIALOG_ABOUT:
                dialog_return = show_dialog(&about_dialog);
                if (dialog_return == 1)
                {
                    dialog_state = DIALOG_EMPTY;//设为无对话框状态
                    show_menu(BLUE);
...
void update_frame(void)
{
    ...
    if (is_edit_open)
    {
        edit_window.is_open = 0;
        show_dialog(&edit_window);
    }
    else
    {
...
void update_menu(MENU *mainmenu,MENU (*submenu)[SUBMENU_LEN],int* subflag)
{
    ...
    for (i =0;i < MENU_LEN;i++)
    {
        if(refresh_menu(mainmenu+i,submenu[i],i,subflag+i))
        {
